...
const LoginPage = () => {
  ...
  React.useEffect(() => {
    if (user) {
      // ✅ Admin redirection first
      if (user.role === 'admin') {
        navigate('/admin/dashboard');
      } 
      // ✅ Subscription check for normal users
      else if (user.packageType === 'none' && !user.isActive && !user.paymentConfirmed && isRegister) {
        navigate('/subscribe');
      } 
      // ✅ Regular active user redirect
      else {
        navigate('/calculator');
      }
    }
